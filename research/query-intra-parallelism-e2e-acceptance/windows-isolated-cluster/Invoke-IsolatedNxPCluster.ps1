...
function ConvertTo-IoTDBPropertyPath([string]$Path) {
  # Java properties treat a backslash as an escape.  Keep native paths for
  # PowerShell operations, but write absolute paths with forward slashes.
  return $Path.Replace('\', '/')
}
...
function Copy-NodeConf([string]$Distribution, [string]$NodeRoot, [System.Collections.IDictionary]$Node, [System.Collections.IDictionary]$ConfigNode, [string]$ClusterName) {
  ...
  $configNodeSystemDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'confignode-system')
  $configNodeConsensusDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'confignode-consensus')
  $configNodePipeDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'confignode-pipe-receiver')
  $dataNodeSystemDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-system')
  $dataNodeDataDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-data')
  $dataNodeConsensusDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-consensus')
  $dataNodeWalDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-wal')
  $dataNodeTracingDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-tracing')
  $dataNodeSyncDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-sync')
  $dataNodePipeDir = ConvertTo-IoTDBPropertyPath (Join-Path $data 'datanode-pipe-receiver')
  $properties = @(
    ...
    "cn_system_dir=$configNodeSystemDir",
    "cn_consensus_dir=$configNodeConsensusDir",
    "cn_pipe_receiver_file_dir=$configNodePipeDir",
    ...
    "dn_system_dir=$dataNodeSystemDir",
    "dn_data_dirs=$dataNodeDataDir",
    "dn_consensus_dir=$dataNodeConsensusDir",
    "dn_wal_dirs=$dataNodeWalDir",
    "dn_tracing_dir=$dataNodeTracingDir",
    "dn_sync_dir=$dataNodeSyncDir",
    "dn_pipe_receiver_file_dirs=$dataNodePipeDir",
...
function Get-RuntimeProcessId([int]$LauncherProcessId, [string]$ExpectedRoot) {
  # On Windows the JVM may detach from the short-lived launcher returned by
  # Start-Process. Record the direct java child when it is uniquely identifiable.
  $children = @(
    Get-CimInstance Win32_Process -Filter "ParentProcessId = $LauncherProcessId" -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -ieq 'java.exe' -and $_.CommandLine -like "*$ExpectedRoot*" })
  if ($children.Count -eq 1) { return [int]$children[0].ProcessId }
  return $LauncherProcessId
}

function Get-QualifiedProcessTree([int]$RootProcessId, [string]$ExpectedRoot) {
  ...
  if ($rootProcess.CommandLine -notlike "*$ExpectedRoot*") {
    throw "Refusing to stop PID ${RootProcessId}: command line no longer identifies this isolated root."
  }
  ...
  while ($queue.Count -gt 0) {
    $entry = $queue.Dequeue()
    $qualified += $entry
    $children = @(Get-CimInstance Win32_Process -Filter "ParentProcessId = $($entry.process.ProcessId)" -ErrorAction SilentlyContinue)
    foreach ($child in $children) {
      # java.exe can create a console-host child which has no deployment path;
      # it is not an IoTDB service and is deliberately left out of the stop set.
      if ($child.Name -ieq 'java.exe' -and $child.CommandLine -like "*$ExpectedRoot*") {
...
function Start-Node([pscustomobject]$Node, [pscustomobject]$Manifest, [string]$ClassPath, [int]$TimeoutSeconds) {
  ...
  $logFileName = if ($role -eq 'ConfigNode') { 'logback-confignode.xml' } else { 'logback-datanode.xml' }
  $logConfig = Join-Path $conf $logFileName
  # ConfigNode uses CONFIGNODE_* properties; DataNode's launcher and descriptors
  # use IOTDB_* (not DATANODE_*). Mirror the supported Windows launchers.
  $propertyPrefix = if ($role -eq 'ConfigNode') { 'CONFIGNODE' } else { 'IOTDB' }
  $properties = @(
    ...
    "-D${propertyPrefix}_HOME=$($Manifest.distribution_root)",
    "-D${propertyPrefix}_DATA_HOME=$data",
    "-D${propertyPrefix}_CONF=$conf",
    "-DTSFILE_HOME=$($Manifest.distribution_root)",
    "-DTSFILE_CONF=$conf",
    "-D${propertyPrefix}_LOGS=$logs",
    "-D${propertyPrefix}_LOG_DIR=$logs",
  ...
  $runtimeProcessId = Get-RuntimeProcessId $process.Id $root
  $pidPath = Join-Path $root 'process.json'
  [ordered]@{ pid = $runtimeProcessId; launcher_pid = $process.Id; class = $class; started_at_utc = [DateTime]::UtcNow.ToString('o'); root = $root } |
...
function Stop-Node([pscustomobject]$Node, [string]$ExpectedRoot) {
  ...
  $qualified = @(Get-QualifiedProcessTree ([int]$record.pid) $ExpectedRoot)
  foreach ($entry in @($qualified | Sort-Object depth -Descending)) {
    Stop-Process -Id $entry.process.ProcessId -Force -ErrorAction SilentlyContinue
  }
  return [ordered]@{ node = $Node.name; stopped = $true; pids = @($qualified | ForEach-Object { $_.process.ProcessId }) }
}
...
$status = foreach ($node in $selected) {
  ...
  if (Test-Path -LiteralPath $processPath) {
    $processId = (Get-Content -LiteralPath $processPath -Raw | ConvertFrom-Json).pid
    $running = $null -ne (Get-Process -Id $processId -ErrorAction SilentlyContinue)
  }
  $expectedListenPort = if ($node.role -eq 'ConfigNode') { $node.internal } else { $node.rpc }
  [ordered]@{ deployment = $node.deployment; node = $node.name; role = $node.role; pid = $processId; process_running = $running; expected_listen_port = $expectedListenPort; stderr = (Join-Path ([string]$node.root) 'logs\stderr.log') }
}
