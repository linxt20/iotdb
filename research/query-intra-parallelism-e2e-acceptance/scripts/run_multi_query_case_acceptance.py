...
def capture_context(output: Path, configs: list[Path]) -> None:
    ...
    for config in configs:
        ...
        if destination.exists():
            raise ValueError(f"configuration basenames must be unique: {config}")
...
def main() -> int:
    ...
    raw_dir.mkdir()
    canonical_dir.mkdir()
    static_cases = {"scan_filter": False, "filter_project": False, "ordered_scan": True, "top_k": True}
    ...
    try:
        ...
        for case_id, ordered in static_cases.items():
            ...
            require_marker(enabled_plan, args.property_marker, f"{case_id} property trace")
            plans[case_id] = {"enabled_plan": str(enabled_plan), "property_marker": args.property_marker}
